...
double bench_granularity(int samples, double granularity) {
  FLAGS_granularity = granularity;
  ...
  for (int i = 0; i < p.baseframe().length(); i += 5) {
    obstacles.push_back(get_cone(planner::point(i, 5)));
    obstacles.push_back(get_cone(planner::point(i, -5)));
  }
...
int main(int argc, char *argv[]) {
  std::ofstream os("bench_granularity.tex");
  ...
  for (double i = 0.1; i < 5; i += 0.1) {
    double time = bench_granularity(100, i);
    std::cout << i << " : " << time << '\n';
    os << boost::format("(%d,%d)\n") % i % time;
  }
  os << "};";
